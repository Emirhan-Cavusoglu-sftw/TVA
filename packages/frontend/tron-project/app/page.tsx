...
export default function Home() {
  ...
  useEffect(() => {
    const fetchAccountAddress = async () => {
      ...
      if (address) {
        ...
        const userHasFund = await entryPointContract.read.balanceOf([address]);
  ...
  const consoleAccount = async () => {
    const account = await readContract(config,{
      ...
      args: ["0xdbbEe24A218185e81Dff47353cb3f3Ccd072ded5"],
    });
    console.log(account);
  }
  ...
  const createAccount = async () => {
    ...
    try {
      const result = await tronWeb.transactionBuilder.triggerSmartContract(
        ...
        { _isConstant: false },
        [{ type: "address", value: (window as any).tronWeb.defaultAddress.base58 }, { type: "string", value: primaryWallet?.address }]
      );
      console.log(result);
      const signedTransaction = await sign(result);
      const transaction = await sendRawTransaction(signedTransaction);
